"use client";

import React, { useEffect } from 'react';
import { motion } from 'framer-motion';

function cn(...classes) {
  return classes.filter(Boolean).join(' ');
}

const sizeMap = {
  sm: "h-12 w-12",
  md: "h-14 w-14",
  lg: "h-16 w-16",
};

function AnimatedCard({ className, title, description, icons = [] }) {
  const maskContainerRef = React.useRef<HTMLDivElement>(null);
  
  useEffect(() => {
    if (maskContainerRef.current) {
      const computed = window.getComputedStyle(maskContainerRef.current);
      // #region agent log
      fetch('http://127.0.0.1:7243/ingest/6ef4e2ef-4865-4b39-ad87-4a73e8e46e89',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'ServicesSection.tsx:34',message:'Mask container styles',data:{maskImage:computed.maskImage,width:computed.width,height:computed.height,zIndex:computed.zIndex,overflow:computed.overflow},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
      // #endregion
    }
  }, []);
  
  return (
    <div
      className={cn(
        "w-full p-6 md:p-8 rounded-xl border-2 border-gray-200 bg-white shadow-md hover:shadow-lg transition-shadow duration-300",
        className
      )}
    >
      <div
        ref={maskContainerRef}
        className={cn(
          "h-[150px] md:h-[180px] rounded-lg relative bg-gradient-to-br from-neutral-200 to-neutral-300 mb-5 overflow-hidden"
        )}
      >
        <div className="relative h-full w-full flex items-center justify-center">
          <AnimatedIcons icons={icons} />
        </div>
      </div>
      {title && (
        <h3 className="text-xl md:text-2xl font-bold text-gray-900 mb-3">
          {title}
        </h3>
      )}
      {description && (
        <p className="text-sm md:text-base font-normal text-gray-600 leading-relaxed">
          {description}
        </p>
      )}
    </div>
  );
}

function AnimatedIcons({ icons }) {
  const scale = [1, 1.1, 1];
  const transform = ["translateY(0px)", "translateY(-4px)", "translateY(0px)"];
  const iconsContainerRef = React.useRef<HTMLDivElement>(null);
  
  useEffect(() => {
    // #region agent log
    fetch('http://127.0.0.1:7243/ingest/6ef4e2ef-4865-4b39-ad87-4a73e8e46e89',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'ServicesSection.tsx:56',message:'AnimatedIcons render',data:{iconCount:icons.length,iconSizes:icons.map(i=>i.size)},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'D'})}).catch(()=>{});
    // #endregion
    
    if (iconsContainerRef.current) {
      const computed = window.getComputedStyle(iconsContainerRef.current);
      const containerRect = iconsContainerRef.current.getBoundingClientRect();
      // #region agent log
      fetch('http://127.0.0.1:7243/ingest/6ef4e2ef-4865-4b39-ad87-4a73e8e46e89',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'ServicesSection.tsx:62',message:'Icons container dimensions',data:{width:containerRect.width,height:containerRect.height,overflow:computed.overflow,position:computed.position},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
      // #endregion
      
      const iconElements = iconsContainerRef.current.querySelectorAll('[class*="circle-"]');
      const parentRect = iconsContainerRef.current.getBoundingClientRect();
      iconElements.forEach((el, idx) => {
        const rect = el.getBoundingClientRect();
        const computed = window.getComputedStyle(el);
        const svg = el.querySelector('svg');
        const svgRect = svg ? svg.getBoundingClientRect() : null;
        const isVisible = rect.width > 0 && rect.height > 0 && rect.top >= 0 && rect.left >= 0;
        // #region agent log
        fetch('http://127.0.0.1:7243/ingest/6ef4e2ef-4865-4b39-ad87-4a73e8e46e89',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'ServicesSection.tsx:70',message:'Icon element dimensions',data:{index:idx,containerWidth:rect.width,containerHeight:rect.height,svgWidth:svgRect?.width||0,svgHeight:svgRect?.height||0,zIndex:computed.zIndex,visibility:computed.visibility,opacity:computed.opacity,top:rect.top,left:rect.left,parentTop:parentRect.top,parentLeft:parentRect.left,isVisible},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
        // #endregion
      });
    }
  }, [icons]);

  return (
    <div className="h-full w-full relative flex items-center justify-center px-2">
      <div ref={iconsContainerRef} className="flex flex-row flex-nowrap justify-center items-center gap-2 md:gap-3">
        {icons.map((icon, index) => (
          <Container
            key={index}
            className={cn(
              sizeMap[icon.size || "lg"],
              `circle-${index + 1}`,
              icon.className
            )}
            style={{
              animation: `float 0.8s ease-in-out ${index * 0.2}s infinite`
            }}
          >
            {icon.icon}
          </Container>
        ))}
      </div>
      <AnimatedSparkles />
      <style>{`
        @keyframes float {
          0%, 100% { transform: translateY(0px) scale(1); }
          50% { transform: translateY(-4px) scale(1.1); }
        }
        @keyframes move {
          0% { transform: translateY(-200px); }
          100% { transform: translateY(200px); }
        }
      `}</style>
    </div>
  );
}

const Container = React.forwardRef(({ className, style, ...props }, ref) => {
  const containerRef = React.useRef<HTMLDivElement>(null);
  const combinedRef = (el: HTMLDivElement | null) => {
    if (typeof ref === 'function') ref(el);
    else if (ref) ref.current = el;
    containerRef.current = el;
  };
  
  useEffect(() => {
    if (containerRef.current) {
      const computed = window.getComputedStyle(containerRef.current);
      const rect = containerRef.current.getBoundingClientRect();
      const svg = containerRef.current.querySelector('svg');
      const svgRect = svg ? svg.getBoundingClientRect() : null;
      // #region agent log
      fetch('http://127.0.0.1:7243/ingest/6ef4e2ef-4865-4b39-ad87-4a73e8e46e89',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'ServicesSection.tsx:100',message:'Container rendered',data:{width:rect.width,height:rect.height,padding:computed.padding,svgExists:!!svg,svgWidth:svgRect?.width||0,svgHeight:svgRect?.height||0},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
      // #endregion
    }
  }, []);
  
  return (
    <div
      ref={combinedRef}
      className={cn(
        `rounded-full flex items-center justify-center bg-white shadow-lg`,
        className
      )}
      style={{
        padding: '10px',
        ...style
      }}
      {...props}
    />
  );
});
Container.displayName = "Container";

const AnimatedSparkles = () => (
  <div 
    className="h-40 w-px absolute top-20 m-auto z-40 bg-gradient-to-b from-transparent via-cyan-500 to-transparent"
    style={{ animation: 'move 3s linear infinite' }}
  >
    <div className="w-10 h-32 top-1/2 -translate-y-1/2 absolute -left-10">
      <Sparkles />
    </div>
  </div>
);

const Sparkles = () => {
  const randomMove = () => Math.random() * 2 - 1;
  const randomOpacity = () => Math.random();
  const random = () => Math.random();

  return (
    <div className="absolute inset-0">
      {[...Array(12)].map((_, i) => (
        <motion.span
          key={`star-${i}`}
          animate={{
            top: `calc(${random() * 100}% + ${randomMove()}px)`,
            left: `calc(${random() * 100}% + ${randomMove()}px)`,
            opacity: randomOpacity(),
            scale: [1, 1.2, 0],
          }}
          transition={{
            duration: random() * 2 + 4,
            repeat: Infinity,
            ease: "linear",
          }}
          style={{
            position: "absolute",
            top: `${random() * 100}%`,
            left: `${random() * 100}%`,
            width: `2px`,
            height: `2px`,
            borderRadius: "50%",
            zIndex: 1,
          }}
          className="inline-block bg-black"
        />
      ))}
    </div>
  );
};

// Logo Components
const ClaudeLogo = ({ className }) => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      shapeRendering="geometricPrecision"
      textRendering="geometricPrecision"
      imageRendering="optimizeQuality"
      fillRule="evenodd"
      clipRule="evenodd"
      viewBox="0 0 512 512"
      className={className}
    >
      <rect fill="#CC9B7A" width="512" height="512" rx="104.187" ry="105.042" />
      <path
        fill="#1F1F1E"
        fillRule="nonzero"
        d="M318.663 149.787h-43.368l78.952 212.423 43.368.004-78.952-212.427zm-125.326 0l-78.952 212.427h44.255l15.932-44.608 82.846-.004 16.107 44.612h44.255l-79.126-212.427h-45.317zm-4.251 128.341l26.91-74.701 27.083 74.701h-53.993z"
      />
    </svg>
  );
};

const CopilotLogo = ({ className }) => {
  return (
    <svg 
      viewBox="0 0 512 416" 
      xmlns="http://www.w3.org/2000/svg" 
      fillRule="evenodd" 
      clipRule="evenodd" 
      strokeLinejoin="round" 
      strokeMiterlimit="2"
      className={className}
    >
      <path fill="currentColor" d="M181.33 266.143c0-11.497 9.32-20.818 20.818-20.818 11.498 0 20.819 9.321 20.819 20.818v38.373c0 11.497-9.321 20.818-20.819 20.818-11.497 0-20.818-9.32-20.818-20.818v-38.373zM308.807 245.325c-11.477 0-20.798 9.321-20.798 20.818v38.373c0 11.497 9.32 20.818 20.798 20.818 11.497 0 20.818-9.32 20.818-20.818v-38.373c0-11.497-9.32-20.818-20.818-20.818z" fillRule="nonzero"/>
      <path fill="currentColor" d="M512.002 246.393v57.384c-.02 7.411-3.696 14.638-9.67 19.011C431.767 374.444 344.695 416 256 416c-98.138 0-196.379-56.542-246.33-93.21-5.975-4.374-9.65-11.6-9.671-19.012v-57.384a35.347 35.347 0 016.857-20.922l15.583-21.085c8.336-11.312 20.757-14.31 33.98-14.31 4.988-56.953 16.794-97.604 45.024-127.354C155.194 5.77 226.56 0 256 0c29.441 0 100.807 5.77 154.557 62.722 28.19 29.75 40.036 70.401 45.025 127.354 13.263 0 25.602 2.936 33.958 14.31l15.583 21.127c4.476 6.077 6.878 13.345 6.878 20.88zm-97.666-26.075c-.677-13.058-11.292-18.19-22.338-21.824-11.64 7.309-25.848 10.183-39.46 10.183-14.454 0-41.432-3.47-63.872-25.869-5.667-5.625-9.527-14.454-12.155-24.247a212.902 212.902 0 00-20.469-1.088c-6.098 0-13.099.349-20.551 1.088-2.628 9.793-6.509 18.622-12.155 24.247-22.4 22.4-49.418 25.87-63.872 25.87-13.612 0-27.86-2.855-39.501-10.184-11.005 3.613-21.558 8.828-22.277 21.824-1.17 24.555-1.272 49.11-1.375 73.645-.041 12.318-.082 24.658-.288 36.976.062 7.166 4.374 13.818 10.882 16.774 52.97 24.124 103.045 36.278 149.137 36.278 46.01 0 96.085-12.154 149.014-36.278 6.508-2.956 10.84-9.608 10.881-16.774.637-36.832.124-73.809-1.642-110.62h.041zM107.521 168.97c8.643 8.623 24.966 14.392 42.56 14.392 13.448 0 39.03-2.874 60.156-24.329 9.28-8.951 15.05-31.35 14.413-54.079-.657-18.231-5.769-33.28-13.448-39.665-8.315-7.371-27.203-10.574-48.33-8.644-22.399 2.238-41.267 9.588-50.875 19.833-20.798 22.728-16.323 80.317-4.476 92.492zm130.556-56.008c.637 3.51.965 7.35 1.273 11.517 0 2.875 0 5.77-.308 8.952 6.406-.636 11.847-.636 16.959-.636s10.553 0 16.959.636c-.329-3.182-.329-6.077-.329-8.952.329-4.167.657-8.007 1.294-11.517-6.735-.637-12.812-.965-17.924-.965s-11.21.328-17.924.965zm49.275-8.008c-.637 22.728 5.133 45.128 14.413 54.08 21.105 21.454 46.708 24.328 60.155 24.328 17.596 0 33.918-5.769 42.561-14.392 11.847-12.175 16.322-69.764-4.476-92.492-9.608-10.245-28.476-17.595-50.875-19.833-21.127-1.93-40.015 1.273-48.33 8.644-7.679 6.385-12.791 21.434-13.448 39.665z"/>
    </svg>
  );
};

const OpenAILogo = ({ className }) => {
  return (
    <svg
      className={className}
      width="28"
      viewBox="0 0 28 28"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M26.153 11.46a6.888 6.888 0 0 0-.608-5.73 7.117 7.117 0 0 0-3.29-2.93 7.238 7.238 0 0 0-4.41-.454 7.065 7.065 0 0 0-2.41-1.742A7.15 7.15 0 0 0 12.514 0a7.216 7.216 0 0 0-4.217 1.346 7.061 7.061 0 0 0-2.603 3.539 7.12 7.12 0 0 0-2.734 1.188A7.012 7.012 0 0 0 .966 8.268a6.979 6.979 0 0 0 .88 8.273 6.89 6.89 0 0 0 .607 5.729 7.117 7.117 0 0 0 3.29 2.93 7.238 7.238 0 0 0 4.41.454 7.061 7.061 0 0 0 2.409 1.742c.92.404 1.916.61 2.923.604a7.215 7.215 0 0 0 4.22-1.345 7.06 7.06 0 0 0 2.605-3.543 7.116 7.116 0 0 0 2.734-1.187 7.01 7.01 0 0 0 1.993-2.196 6.978 6.978 0 0 0-.884-8.27Zm-10.61 14.71c-1.412 0-2.505-.428-3.46-1.215.043-.023.119-.064.168-.094l5.65-3.22a.911.911 0 0 0 .464-.793v-7.86l2.389 1.36a.087.087 0 0 1 .046.065v6.508c0 2.952-2.491 5.248-5.257 5.248ZM4.062 21.354a5.17 5.17 0 0 1-.635-3.516c.042.025.115.07.168.1l5.65 3.22a.928.928 0 0 0 .928 0l6.898-3.93v2.72a.083.083 0 0 1-.034.072l-5.711 3.255a5.386 5.386 0 0 1-4.035.522 5.315 5.315 0 0 1-3.23-2.443ZM2.573 9.184a5.283 5.283 0 0 1 2.768-2.301V13.515a.895.895 0 0 0 .464.793l6.897 3.93-2.388 1.36a.087.087 0 0 1-.08.008L4.52 16.349a5.262 5.262 0 0 1-2.475-3.185 5.192 5.192 0 0 1 .527-3.98Zm19.623 4.506-6.898-3.93 2.388-1.36a.087.087 0 0 1 .08-.008l5.713 3.255a5.28 5.28 0 0 1 2.054 2.118 5.19 5.19 0 0 1-.488 5.608 5.314 5.314 0 0 1-2.39 1.742v-6.633a.896.896 0 0 0-.459-.792Zm2.377-3.533a7.973 7.973 0 0 0-.168-.099l-5.65-3.22a.93.93 0 0 0-.928 0l-6.898 3.93V8.046a.083.083 0 0 1 .034-.072l5.712-3.251a5.375 5.375 0 0 1 5.698.241 5.262 5.262 0 0 1 1.865 2.28c.39.92.506 1.93.335 2.913ZM9.631 15.009l-2.39-1.36a.083.083 0 0 1-.046-.065V7.075c.001-.997.29-1.973.832-2.814a5.297 5.297 0 0 1 2.231-1.935 5.382 5.382 0 0 1 5.659.72 4.89 4.89 0 0 0-.168.093l-5.65 3.22a.913.913 0 0 0-.465.793l-.003 7.857Zm1.297-2.76L14 10.5l3.072 1.75v3.5L14 17.499l-3.072-1.75v-3.5Z"
        fill="currentColor"
      />
    </svg>
  );
};

const GeminiLogo = ({ className }) => {
  return (
    <svg
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 16 16"
      className={className}
    >
      <path
        d="M16 8.016A8.522 8.522 0 008.016 16h-.032A8.521 8.521 0 000 8.016v-.032A8.521 8.521 0 007.984 0h.032A8.522 8.522 0 0016 7.984v.032z"
        fill="url(#prefix__paint0_radial_980_20147)"
      />
      <defs>
        <radialGradient
          id="prefix__paint0_radial_980_20147"
          cx="0"
          cy="0"
          r="1"
          gradientUnits="userSpaceOnUse"
          gradientTransform="matrix(16.1326 5.4553 -43.70045 129.2322 1.588 6.503)"
        >
          <stop offset=".067" stopColor="#9168C0" />
          <stop offset=".343" stopColor="#5684D1" />
          <stop offset=".672" stopColor="#1BA1E3" />
        </radialGradient>
      </defs>
    </svg>
  );
};

const MetaIconOutline = ({ className }) => {
  return (
    <svg
      id="Layer_1"
      data-name="Layer 1"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 287.56 191"
      className={className}
    >
      <defs>
        <linearGradient
          id="linear-gradient"
          x1="62.34"
          y1="101.45"
          x2="260.34"
          y2="91.45"
          gradientTransform="matrix(1, 0, 0, -1, 0, 192)"
          gradientUnits="userSpaceOnUse"
        >
          <stop offset="0" stopColor="#0064e1" />
          <stop offset="0.4" stopColor="#0064e1" />
          <stop offset="0.83" stopColor="#0073ee" />
          <stop offset="1" stopColor="#0082fb" />
        </linearGradient>
        <linearGradient
          id="linear-gradient-2"
          x1="41.42"
          y1="53"
          x2="41.42"
          y2="126"
          gradientTransform="matrix(1, 0, 0, -1, 0, 192)"
          gradientUnits="userSpaceOnUse"
        >
          <stop offset="0" stopColor="#0082fb" />
          <stop offset="1" stopColor="#0064e0" />
        </linearGradient>
      </defs>
      <path
        fill="#0081fb"
        d="M31.06,126c0,11,2.41,19.41,5.56,24.51A19,19,0,0,0,53.19,160c8.1,0,15.51-2,29.79-21.76,11.44-15.83,24.92-38,34-52l15.36-23.6c10.67-16.39,23-34.61,37.18-47C181.07,5.6,193.54,0,206.09,0c21.07,0,41.14,12.21,56.5,35.11,16.81,25.08,25,56.67,25,89.27,0,19.38-3.82,33.62-10.32,44.87C271,180.13,258.72,191,238.13,191V160c17.63,0,22-16.2,22-34.74,0-26.42-6.16-55.74-19.73-76.69-9.63-14.86-22.11-23.94-35.84-23.94-14.85,0-26.8,11.2-40.23,31.17-7.14,10.61-14.47,23.54-22.7,38.13l-9.06,16c-18.2,32.27-22.81,39.62-31.91,51.75C84.74,183,71.12,191,53.19,191c-21.27,0-34.72-9.21-43-23.09C3.34,156.6,0,141.76,0,124.85Z"
      />
      <path
        fill="url(#linear-gradient)"
        d="M24.49,37.3C38.73,15.35,59.28,0,82.85,0c13.65,0,27.22,4,41.39,15.61,15.5,12.65,32,33.48,52.63,67.81l7.39,12.32c17.84,29.72,28,45,33.93,52.22,7.64,9.26,13,12,19.94,12,17.63,0,22-16.2,22-34.74l27.4-.86c0,19.38-3.82,33.62-10.32,44.87C271,180.13,258.72,191,238.13,191c-12.8,0-24.14-2.78-36.68-14.61-9.64-9.08-20.91-25.21-29.58-39.71L146.08,93.6c-12.94-21.62-24.81-37.74-31.68-45C107,40.71,97.51,31.23,82.35,31.23c-12.27,0-22.69,8.61-31.41,21.78Z"
      />
      <path
        fill="url(#linear-gradient-2)"
        d="M82.35,31.23c-12.27,0-22.69,8.61-31.41,21.78C38.61,71.62,31.06,99.34,31.06,126c0,11,2.41,19.41,5.56,24.51L10.14,167.91C3.34,156.6,0,141.76,0,124.85,0,94.1,8.44,62.05,24.49,37.3,38.73,15.35,59.28,0,82.85,0Z"
      />
    </svg>
  );
};

// Demo Component with 4 Cards
export default function ServicesSection() {
  return (
    <section className="relative w-full bg-white py-16 md:py-20 lg:py-24">
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12 md:mb-16">
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 mb-4">Services</h2>
          <p className="text-gray-600 text-base md:text-lg lg:text-xl">Comprehensive performance marketing solutions</p>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8" style={{ maxWidth: '1024px', margin: '0 auto' }}>
          {/* Card 1 */}
          <AnimatedCard
            title="Damn good card"
            description="A card that showcases a set of tools that you use to create your product."
            icons={[
              {
                icon: <ClaudeLogo className="h-10 w-10" />,
                size: "sm",
              },
              {
                icon: <CopilotLogo className="h-12 w-12 text-gray-800" />,
                size: "md",
              },
              {
                icon: <OpenAILogo className="h-14 w-14 text-gray-800" />,
                size: "lg",
              },
              {
                icon: <MetaIconOutline className="h-12 w-12" />,
                size: "md",
              },
              {
                icon: <GeminiLogo className="h-10 w-10" />,
                size: "sm",
              },
            ]}
          />

          {/* Card 2 */}
          <AnimatedCard
            title="Damn good card"
            description="A card that showcases a set of tools that you use to create your product."
            icons={[
              {
                icon: <ClaudeLogo className="h-10 w-10" />,
                size: "sm",
              },
              {
                icon: <CopilotLogo className="h-12 w-12 text-gray-800" />,
                size: "md",
              },
              {
                icon: <OpenAILogo className="h-14 w-14 text-gray-800" />,
                size: "lg",
              },
              {
                icon: <MetaIconOutline className="h-12 w-12" />,
                size: "md",
              },
              {
                icon: <GeminiLogo className="h-10 w-10" />,
                size: "sm",
              },
            ]}
          />

          {/* Card 3 */}
          <AnimatedCard
            title="Damn good card"
            description="A card that showcases a set of tools that you use to create your product."
            icons={[
              {
                icon: <ClaudeLogo className="h-10 w-10" />,
                size: "sm",
              },
              {
                icon: <CopilotLogo className="h-12 w-12 text-gray-800" />,
                size: "md",
              },
              {
                icon: <OpenAILogo className="h-14 w-14 text-gray-800" />,
                size: "lg",
              },
              {
                icon: <MetaIconOutline className="h-12 w-12" />,
                size: "md",
              },
              {
                icon: <GeminiLogo className="h-10 w-10" />,
                size: "sm",
              },
            ]}
          />

          {/* Card 4 */}
          <AnimatedCard
            title="Damn good card"
            description="A card that showcases a set of tools that you use to create your product."
            icons={[
              {
                icon: <ClaudeLogo className="h-10 w-10" />,
                size: "sm",
              },
              {
                icon: <CopilotLogo className="h-12 w-12 text-gray-800" />,
                size: "md",
              },
              {
                icon: <OpenAILogo className="h-14 w-14 text-gray-800" />,
                size: "lg",
              },
              {
                icon: <MetaIconOutline className="h-12 w-12" />,
                size: "md",
              },
              {
                icon: <GeminiLogo className="h-10 w-10" />,
                size: "sm",
              },
            ]}
          />
        </div>
      </div>
    </section>
  );
}
